...
static func for_location(location_id: String) -> Array:
	match location_id:
		...
		"hill":  # 丘で過ごす（町を見下ろす）
			return [
				{ "speaker": "", "text": "丘の上から、町の全部が見渡せる。褪せかけた空の下、川がきらめいている。" },
				{ "speaker": PLAYER, "text": "……ここから見ると、ぜんぶ、ちいさいな。" },
				{ "speaker": "", "text": "終わりを見晴らす場所で、ひとり、風に吹かれていた。" },
			]
		"school":  # 無人の校舎で過ごす（夏休みの空白）
			return [
				{ "speaker": "", "text": "夏休みの校舎は、しんと静まりかえっている。誰もいない廊下に、足音だけが響く。" },
				{ "speaker": PLAYER, "text": "……こんなに静かだったっけ。" },
				{ "speaker": "", "text": "何をするでもなく、空っぽの教室をぶらついた。それも、夏の一日だ。" },
			]
		_:  # 家で過ごす
			return [
...
static func aoi_ambient() -> Array:
	return [
		...
		# 所持品：葵は道具を手渡さない。会ったあと、いつのまにか手元にある（貝殻）。由来は空（所持品システム §5-1）。
		#   ＝「もらう」でなく「残る」。add_item("aoi_shell") の origin は Items マスタで空（喪失後も消さない）。
		{ "effect": { "item": "aoi_shell" } },
	]
