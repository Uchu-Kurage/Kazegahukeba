...
# --- フェーズの日付範囲（day_index。0 = 7/23、39 = 8/31）------------------------
## 各ルートの節目データ（Routes.gd）が since/until にこれらを使う。
## 前奏（7/23〜7/31）＋八月序盤を序盤に、祭り(8/20)・選択を中盤に、台風(8/23〜)以降の
## 喪失・惜別・着地を終盤に置く（劇的な流れが 8/31 の着地へ向かう）。
const EARLY_START := 0
const EARLY_END := 15    # 序盤 7/23〜8/7 ごろ（前奏＋出会い直し）
const MID_START := 16
const MID_END := 30      # 中盤 8/8〜8/22 ごろ（深まり・選択・祭り8/20）
const LATE_START := 31
const LATE_END := 39     # 終盤 8/23〜8/31（喪失・決壊・最後の日々・着地）
