...
## 40日ぶんの手組みスケジュール（day_index=0 が 7/23＝8/31 の40日前）。設計書の代表案に準拠。
## 構成：前半 7/23〜7/31（day0-8）は「夏の入り口」の前奏、8/1〜8/31（day9-39）に設計済みの
##   起伏（夕立=葵の初影／台風前=喪失の予兆／惜別／着地）を配置し、劇的な山場が 8/31 へ向かう。
## マイルストーンの日付は仮。配置ルールを保つ範囲で前後可。
## 末尾 9/1（day40。通常は到達しない）は裏エンド用「何事もなかったように晴れる残酷さ」＝快晴。
const SCHEDULE := [
	# --- 前奏：夏の入り口（7/23〜7/31）。強い特殊天気は置かず、静かに夏へ入っていく ---
	CLEAR,       # 7/23 夏休みの入り口・目覚め
	CLEAR,       # 7/24
	CLEAR_MAX,   # 7/25 夏本番の予感
	CLOUDY,      # 7/26 天気が動くことを学習させる
	CLEAR,       # 7/27
	RAIN,        # 7/28 通り雨・静かな日
	CLEAR,       # 7/29
	CLEAR,       # 7/30
	CLEAR_MAX,   # 7/31 明日から八月
	# --- 八月：設計済みの起伏（8/1〜8/31）。ここから物語が本格化する ---
	CLEAR,       # 8/1  夏の始まり
	...
	CLEAR,       # 8/27
	CLEAR_MAX,   # 8/28 最後の夏らしい快晴・惜別の眩しさ（＝最後の花火の夜）
	SUNSET,      # 8/29 終わりが目前・蜩の声・郷愁
	CLOUDY,      # 8/30 最後の夜の前の静けさ・心の準備
	CLEAR,       # 8/31 ★着地（三分岐）。実際の天気はエンドが分岐ごとに決める
	CLEAR_MAX,   # 9/1  ura-end：何事もなかったように晴れる残酷さ（通常は到達しない予備）
]

## 山場の日（day_index）。この日の天気は予報を必ず当てる（前日の予報も外さない）。
## ＝設計書「山場の前日は予報を当てる」。特殊天気（夕立/霧/台風/夕焼け）も揺らがせない。
## 8/5=13 / 8/20=28 / 8/23=31 / 8/24=32 / 8/25=33 / 8/29=37 / 8/31=39（＝前奏9日ぶん +9 シフト）。
const KEY_DAYS := [13, 28, 31, 32, 33, 37, 39]
