...
## この値“以下”なら「球磨とほとんど関わらなかった」とみなす（→ 静かな夏へ）。
const KUMA_AFF_MIN := 1
## 寄り添い(STAY_BESIDE)で王道(着地1・友情)に届く関係値の下限（この値“以上”）。
const KUMA_AFF_FRIENDSHIP := 5
...
static func pick(affinity: Dictionary, flags: Dictionary, stance: int) -> String:
	...
	if kuma <= KUMA_AFF_MIN and stance == GameState.KumaStance.NONE:
		return "quiet_summer"
	...
	match stance:
		...
		GameState.KumaStance.STAY_BESIDE:
			return "kuma_friendship" if kuma >= KUMA_AFF_FRIENDSHIP else "kuma_struggle"  # 寄り添い、深く見届けた
		GameState.KumaStance.DISSUADE:
			return "kuma_bitter"                                                      # 諫めた／すれ違い

	# 立場未選択（中盤に至らず）は、好感度で寄せる。
	if kuma <= KUMA_AFF_MIN:
		return "quiet_summer"
